...
class MurderMysteryGame(pyspiel.Game):
    """Very Simple Murder Mystery Game"""

    def __init__(self, params=None) -> None:
        super().__init__(_GAME_TYPE, _GAME_INFO, params or dict())

    def new_initial_state(self) -> MurderMysteryState:
        """Returns a state corresponding to the start of a game."""
        return MurderMysteryState(self)

    def make_py_observer(self, iig_obs_type=None, params=None):
        """Returns an object used for observing game state."""
        return MurderMysteryObserver(
            iig_obs_type or pyspiel.IIGObservationType(perfect_recall=False),
            params)


class MurderMysteryState(pyspiel.State):
    ...
    def n_actions(self) -> int:
        ...
        elif self.to_play() == MurderMysteryPlayer.DETECTIVE:
            return len(self.alive - {self.killer})
        else:
            return len(self.alive - self.accused)

    def is_terminal(self):
        """Returns True if the game is over."""
        return self._move_no >= self._max_turns or self.n_actions() == 0
    ...
    def score(self) -> float:
        ...
        total -= Rewards.COST_PER_DEATH * len(self.dead)
        # note the minus below is set minus - there  is no cost for accusing the killer
        total -= Rewards.COST_PER_ACCUSATION * len(self.accused - {self.killer})
...
class MurderMysteryObserver:
    ...
    def set_from(self, state: MurderMysteryState, player: int):
    ...
    def string_from(self, state: MurderMysteryState, player: int):
